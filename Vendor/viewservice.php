<?php   require "config/database.conf.php"; ?>
 ...
 /*================================SELECT ALL ITEMS FROM ITEM LIST===============*/
   $select = "SELECT * FROM `service` WHERE `owner` = '{$_SESSION['vendor_email']}' LIMIT 20";
...
$url ="upload_pic.php?service_id=".urlencode($id)."&service_name=".urlencode($name);
?>

                <tr>
                <td width="15%"><a href="<?=$url;?>"><img  src="../assets/img/uploads/<?=$image; ?>"  class="img-thumbnail" ></a></td>
                    <td><?= $name; ?></td>
                    <td><?= $category;  ?></td>
                    <td><?= $city; ?></td>
                    <td><?= $county; ?></td>
